...
function live(){
 ...
 return `<section class="live-section"><div class="container"><div class="live-title"><div><span class="live-kicker">GTV MEDIOS</span><h2>Gamarra TV <b>EN VIVO</b></h2></div><span class="live-pill"><i></i> SEÑAL EN DIRECTO</span></div><div class="live-layout"><div class="player-column"><div class="player">${String(C.liveUrl||'').includes('.m3u8')?`<video id="gtvLiveVideo" class="live-video" controls playsinline muted preload="metadata" poster="${esc(fallback)}"></video>`:`<iframe src="${esc(C.liveUrl||'')}" title="Señal en vivo de Gamarra TV" allow="autoplay;fullscreen" allowfullscreen></iframe>`}<div class="player-label"><i></i> GTV EN VIVO</div></div><div class="live-note">Señal abierta de Gamarra TV</div></div><aside class="schedule"><div class="schedule-head"><div><span>HOY EN GTV</span><h3>PROGRAMACIÓN</h3></div><span class="schedule-day">${esc(dayInfo(d)[1])}</span></div><div class="days">${DAYS.map(x=>`<button class="day ${d===x[0]?'active':''}" data-day="${x[0]}">${x[1]}</button>`).join('')}</div><div class="schedule-list">${rowsHtml}</div><div class="next-box">${next?`<span>PRÓXIMO</span><strong>${esc(next.programa)}</strong><small>${esc(time(next.hora_inicio||next.hora))}</small>`:`<span>PROGRAMACIÓN</span><strong>${current?'Al aire en este momento':'Fuera de programación'}</strong>`}</div></aside></div></div></section>`
}
...
function initLivePlayer(){
 const video=document.getElementById('gtvLiveVideo');
 if(!video||!C.liveUrl)return;
 const src=String(C.liveUrl);
 if(video.canPlayType('application/vnd.apple.mpegurl')){video.src=src;}
 else if(window.Hls&&window.Hls.isSupported()){
   const hls=new window.Hls({enableWorker:true,lowLatencyMode:true});
   hls.loadSource(src); hls.attachMedia(video); window.__gtvHls=hls;
   hls.on(window.Hls.Events.MANIFEST_PARSED,()=>video.play().catch(()=>{}));
 } else { video.outerHTML='<div class="player-error">Tu navegador no puede reproducir la señal en vivo.</div>'; }
}
