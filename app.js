...
const S={session:null,news:[],programs:[],clients:[],day:null};
...
function live(){
 ...
 return `<section class="live-section"><div class="container"><div class="live-title"><div><span class="live-kicker">GTV MEDIOS</span><h2>Gamarra TV <b>EN VIVO</b></h2></div><span class="live-pill"><i></i> SEÑAL EN DIRECTO</span></div><div class="live-layout"><div class="player-column"><div class="player" id="gtvPlayer"><video id="gtvVideo" playsinline controls autoplay muted preload="metadata" poster="${esc(fallback)}"></video><div id="gtvFallback" class="live-fallback" hidden><div class="fallback-message"><strong>No se pudo reproducir la señal directa.</strong><p>Conectando con el reproductor de OpenCaster…</p><button type="button" id="retryLive" class="btn btn-primary">🔄 Intentar de nuevo</button></div><iframe id="gtvOpenCaster" title="Gamarra TV en vivo – OpenCaster" allow="autoplay;fullscreen" allowfullscreen loading="lazy"></iframe></div><div class="player-label"><i></i> GTV EN VIVO</div></div><div class="live-note">Señal abierta de Gamarra TV</div></div><aside class="schedule"><div class="schedule-head"><div><span>HOY EN GTV</span><h3>PROGRAMACIÓN</h3></div><span class="schedule-day">${esc(dayInfo(d)[1])}</span></div><div class="days">${DAYS.map(x=>`<button class="day ${d===x[0]?'active':''}" data-day="${x[0]}">${x[1]}</button>`).join('')}</div><div class="schedule-list">${rowsHtml}</div><div class="next-box">${next?`<span>PRÓXIMO</span><strong>${esc(next.programa)}</strong><small>${esc(time(next.hora_inicio||next.hora))}</small>`:`<span>PROGRAMACIÓN</span><strong>${current?'Al aire en este momento':'Fuera de programación'}</strong>`}</div></aside></div></div></section>`
}
function setupLivePlayer(){
 const video=document.getElementById('gtvVideo'),fallback=document.getElementById('gtvFallback'),frame=document.getElementById('gtvOpenCaster'),retry=document.getElementById('retryLive');
 if(!video||!fallback||!frame)return;
 const hlsUrl=C.liveUrl||'https://vs20.live.opencaster.com/gamarratv_9c41922a/index.m3u8';
 const embedUrl='https://new.opencaster.com/player/embed?user=gamarratv';
 let hls=null,switched=false;
 const showFallback=()=>{if(switched)return;switched=true;try{if(hls)hls.destroy()}catch(e){} video.pause();video.removeAttribute('src');fallback.hidden=false;frame.src=embedUrl;};
 const start=()=>{switched=false;fallback.hidden=true;frame.removeAttribute('src');if(hls){try{hls.destroy()}catch(e){}hls=null;}if(video.canPlayType('application/vnd.apple.mpegurl')){video.src=hlsUrl;video.play().catch(()=>{});return;}if(window.Hls&&Hls.isSupported()){hls=new Hls({enableWorker:true,lowLatencyMode:true,backBufferLength:30});hls.loadSource(hlsUrl);hls.attachMedia(video);hls.on(Hls.Events.MANIFEST_PARSED,()=>video.play().catch(()=>{}));hls.on(Hls.Events.ERROR,(_,data)=>{if(data&&data.fatal){if(data.type===Hls.ErrorTypes.MEDIA_ERROR){try{hls.recoverMediaError();return}catch(e){}}showFallback();}});return;}showFallback();};
 video.addEventListener('error',showFallback);if(retry)retry.onclick=start;start();
}
...
function bindLive(){document.querySelectorAll('.day').forEach(b=>b.onclick=()=>{S.day=b.dataset.day;render()});setupLivePlayer()}
...
async function admin(){
 ...
 const news=nr.data||[], programs=pr.data||[], clients=cr.error?[]:(cr.data||[]);
...
async function render(){const hash=location.hash||'#/';try{if(hash.startsWith('#/noticia/')){return article(decodeURIComponent(hash.slice(10)))}if(hash==='#/login')return login();if(hash==='#/admin')return admin();if(hash==='#/contacto')return contact();if(hash==='#/clima')return weatherPage();const cat=hash.startsWith('#/categoria/')?decodeURIComponent(hash.slice(12)).toLowerCase():null;if(hash==='#/noticias'||cat){await loadNews();return newsPage(cat||'')}if(hash==='#/en-vivo'||hash==='#/programacion'){setMeta();await loadPrograms();app.innerHTML=live();bindLive();return}setMeta();await loadNews();await loadPrograms();await loadClients();S.day=null;app.innerHTML=home();bindHome();renderHomeClients()}catch(e){console.error(e);app.innerHTML=`<div class="error-box"><h2>No se pudo cargar el contenido</h2><p>${esc(e.message||e)}</p><a class="back-link" href="#/">← Volver al inicio</a></div>`}}
